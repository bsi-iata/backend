...
public class UploadController : BaseController
{
    ...
    public async Task<UnifyResultDto> Import(IFormFile form)
    {
        ...
        foreach (var item in res.Goods)
        {
            ...
            if (item.Commodity  == "fireworks")
            {
                var msg = GetMessageByMimeType("1", item.Commodity);
                item.AiResult = msg;
                res.AIResults.Add(msg);

            }
            else if (item.Commodity == "mobile")
            {
                var msg = GetMessageByMimeType("5", item.Commodity);
                item.AiResult = msg;
                res.AIResults.Add(msg);
            }

            else if (item.Commodity == "latop")
            {
                var msg = GetMessageByMimeType("8", item.Commodity);
                item.AiResult = msg;
                res.AIResults.Add(msg);
            }
            else if (item.Commodity == "perfume")
            {
                var msg = GetMessageByMimeType("6", item.Commodity);
                item.AiResult = msg;
                res.AIResults.Add(msg);
            }
        }
    ...
    private string GetMessageByMimeType(string type, string goodName)
    {
        return type switch
        {
            "1" => $"Seems the goods {goodName} is Class 1 Explosives Dangerous goods(IATA DGR).<br/>Usually this kind of goods are not allowed to tranport by major of airlines.<br/>Please contact Customer Service for further guiding declaration.",
            "2" => $"Seems the goods {goodName} is Class 2 Gases Dangerous goods(IATA DGR).<br/>Usually this kind of goods are not allowed to tranport by major of airlines.<br/>Please contact Customer Service for further guiding declaration.",
            "3" => $"Seems the goods {goodName} is Class 3 Flammable liquids Dangerous goods(IATA DGR).<br/>Please contact Customer Service for further guiding declaration.",
            "4" => $"Seems the goods {goodName} is Class 4 Flammable solids Dangerous goods(IATA DGR).<br/>Please contact Customer Service for further guiding declaration.",
            "5" => $"Seems the goods {goodName} is Class 5 Oxidizing substances or organic peroxides Dangerous goods(IATA DGR).<br/>Please contact Customer Service for further guiding declaration.",
            "6" => $"Seems the goods {goodName} is Class 6 Toxic or infectious substances Dangerous goods(IATA DGR).<br/>Please contact Customer Service for further guiding declaration.",
            "7" => $"Seems the goods {goodName} is Class 7 Radioactive material Dangerous goods(IATA DGR).<br/>Please contact Customer Service for further guiding declaration.",
            "8" => $"Seems the goods {goodName} is Class 8 Corrosive substances Dangerous goods(IATA DGR).<br/>Please contact Customer Service for further guiding declaration.",
            "9" => $"Seems the goods {goodName} is Class 9 Miscellaneous dangerous substances and articles(IATA DGR).<br/>Please refer to following criteria for shipping dangerous goods:<br/>1. Providing Material Safety Data Sheet (MSDS)?<br/>2. Providing UN/DOT 38.3, testing helps ensure the safety of lithium ion or lithium metal batteries during shipping.",
            _ => ""
        };
    }
